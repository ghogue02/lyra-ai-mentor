...
class GamificationService {
  ...
  async loadProgress(): Promise<void> {
    try {
      ...
      if (savedProgress) {
        ...
        this.progressData = this.createNewProgress(user?.id || 'anonymous');
      }
    } catch (error) {
  ...
  async saveProgress(): Promise<void> {
    ...
    this.saveTimeout = setTimeout(async () => {
      try {
        ...
        const { data: { user } } = await supabase.auth.getUser();
        if (user && this.progressData) {
          const { error } = await supabase
            .from('user_progress')
            .upsert({
              user_id: user.id,
              completed_components: this.progressData.completedComponents,
              time_spent_by_component: this.progressData.timeSpentByComponent,
              skills_mastered: this.progressData.skillsMastered,
              current_streak: this.progressData.currentStreak,
              longest_streak: this.progressData.longestStreak,
              last_active_date: this.progressData.lastActiveDate.toISOString(),
              total_time_spent: this.progressData.totalTimeSpent,
              badges: this.progressData.badges,
              level: this.progressData.level,
              experience: this.progressData.experience,
              next_level_experience: this.progressData.nextLevelExperience,
  ...
  getBadges(): Badge[] {
    return this.BADGES.map(badge => ({
      ...
      unlockedAt: this.progressData?.badges.find(b => b.id === badge.id)?.unlockedAt
    }));
  }
  ...
  getComponentProgress(componentId: string): ComponentProgress | null {
    ...
    const isCompleted = this.progressData.completedComponents.includes(componentId);
  ...
  getSuggestedNextComponent(): string | null {
    ...
    // Find incomplete components by character
    const characterComponents: Record<string, string[]> = {
      maya: ['MayaEmailComposer', 'MayaCommunicationCoach', 'MayaConfidenceBuilder'],
      david: ['DavidDataStoryFinder', 'DavidDataVisualizer', 'DavidInsightGenerator'],
      rachel: ['RachelAutomationVision', 'RachelProcessMapper', 'RachelTaskAutomator'],
      alex: ['AlexChangeStrategy', 'AlexStrategicPlanning', 'AlexLeadershipDevelopment'],
      sofia: ['SofiaVoiceDiscovery', 'SofiaStoryCreator', 'SofiaAuthenticityChecker']
    };

    // Find character with least progress
    let leastProgressChar = '';
    let leastProgress = 1;

    Object.entries(characterComponents).forEach(([char, components]) => {
      const completed = components.filter(c => 
        this.progressData!.completedComponents.includes(c)
      ).length;
      const progress = completed / components.length;
      
      if (progress < leastProgress) {
        leastProgress = progress;
        leastProgressChar = char;
      }
    });

    // Return first incomplete component from least progress character
    const incompleteComponents = characterComponents[leastProgressChar]?.filter(c => 
      !this.progressData!.completedComponents.includes(c)
    );

    return incompleteComponents?.[0] || null;
  }

  resetProgress(): void {
    const userId = this.progressData?.userId || 'anonymous';
    this.progressData = this.createNewProgress(userId);
    this.saveProgress();
  }
}
