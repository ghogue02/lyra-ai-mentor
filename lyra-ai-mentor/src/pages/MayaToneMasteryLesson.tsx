...
// Utility function to format email content with best practices
const formatEmailContent = (content: string): JSX.Element => {
  if (!content) return <span>No content available</span>;
  
  // Split content into lines and identify email components
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.toLowerCase().startsWith('subject:')) {
      formattedElements.push(
        <div key={currentIndex++} className="mb-3">
          <div className="font-semibold text-gray-800 bg-gray-100 px-2 py-1 rounded">
            {line}
          </div>
        </div>
      );
    ...
    // Greeting
    else if (line.toLowerCase().startsWith('dear ') || line.toLowerCase().startsWith('hi ') || line.toLowerCase().startsWith('hello ')) {
      formattedElements.push(
        <div key={currentIndex++} className="mb-3">
    ...
    // Signature block
    else if (line.toLowerCase().includes('regards') || line.toLowerCase().includes('sincerely') || line.toLowerCase().includes('best') || 
             line.includes('Maya Rodriguez') || line.includes('Program Director')) {
      formattedElements.push(
        <div key={currentIndex++} className="mt-4 pt-2 border-t border-gray-200">
          <div className="text-gray-700">{line}</div>
        </div>
      );
    }
    // Contact info
    else if (line.includes('[Contact') || line.includes('[Website') || line.includes('Hope Gardens Community Center')) {
      formattedElements.push(
        <div key={currentIndex++} className="text-xs text-gray-500 mt-1">
          {line}
        </div>
      );
    }
    // Regular paragraphs
      ...
      formattedElements.push(
        <div key={currentIndex++} className="mb-3">
          <div className="text-gray-700 leading-relaxed">{line}</div>
        </div>
      );
    }
  }
...
// AI Generation Popover Component
interface AIGenerationPopoverProps {
  isVisible: boolean;
...
const AIGenerationPopover: React.FC<AIGenerationPopoverProps> = ({ 
  isVisible, 
  currentPrompt, 
  step, 
  totalSteps, 
  ...
  return (
    ...
    >
      <Card className="border-blue-400 bg-gradient-to-br from-blue-50 to-indigo-100 shadow-xl">
        <CardContent className="p-4">
          <div className="flex items-center gap-3 mb-3">
            <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-600"></div>
            <div>
              <h3 className="font-bold text-blue-900 text-sm">🤖 AI Processing</h3>
              <p className="text-xs text-blue-700">Step {step} of {totalSteps}</p>
            </div>
          </div>
          
          <div className="bg-white/80 rounded-lg p-3 mb-3">
            <p className="text-xs font-medium text-gray-700 mb-1">Current Step:</p>
            <p className="text-xs text-gray-600">{message}</p>
          </div>
          
          <div className="bg-blue-100 rounded-lg p-2">
            <p className="text-xs font-medium text-blue-800 mb-1">
              {step === 1 ? "Generic Prompt:" : step === 2 ? "Maya's Enhanced Prompt:" : "Current Prompt:"}
            </p>
            <p className="text-xs text-blue-700 italic leading-relaxed">{currentPrompt}</p>
          </div>
...
const MayaToneMasteryLesson: React.FC = () => {
  ...
  const [toneMasteryJourney, setToneMasteryJourney] = useState({
    ...
    // Practice AI state
    practiceGenericResponse: '',
    practiceSpecificResponse: '',
  ...
  const generateToneComparison = async (audience: AudienceProfile) => {
    try {
      ...
      // Show popover step 1
      const genericPromptText = `Write a professional email about ${baseScenario}`;
      setAiPopover({
        isVisible: true,
        currentPrompt: genericPromptText,
        step: 1,
        totalSteps: 3,
        message: "Generating generic AI response with basic prompt"
      });
      ...
      const genericPrompt = {
        ...
        tone: "professional",
        aiPrompt: genericPromptText
      };
      
      // Wait for step 1 to be visible (longer for readability)
      await new Promise(resolve => setTimeout(resolve, 3500));
      
      // Show popover step 2
      setAiPopover(prev => ({
        ...prev,
        currentPrompt: `Write a ${audience.preferredTone} email for ${audience.name} who ${audience.context}`,
        step: 2,
        message: "Generating audience-specific response with Maya's technique"
      }));
      ...
      // Wait for step 2 to be visible (longer for readability)
      await new Promise(resolve => setTimeout(resolve, 3500));
      ...
      setAiPopover(prev => ({
        ...prev,
        step: 3,
        message: "Comparing results to show the dramatic difference"
      }));
      ...
      // Wait for step 3 to be visible (shorter since it's brief text)
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      // Hide popover
      setAiPopover(prev => ({ ...prev, isVisible: false }));
      ...
      // Wait for step 3 to be visible before hiding (shorter timing)
      await new Promise(resolve => setTimeout(resolve, 1500));
  ...
  // Generate real AI practice comparison
  const generatePracticeComparison = async (exercise: ToneExercise) => {
    try {
      // Show popover step 1 - Building generic prompt
      const genericPromptText = `Write a professional email about ${exercise.scenario}`;
      setAiPopover({
        isVisible: true,
        currentPrompt: genericPromptText,
        step: 1,
        totalSteps: 4,
        message: "Building generic prompt (scenario-specific but no audience context)"
      });
      
      // Generate generic response
      const genericPrompt = {
        purpose: exercise.scenario,
        audience: "recipient",
        audienceContext: "general person",
        situationDetails: "routine communication",
        tone: "professional",
      ...
      // Show popover step 2 - Building Maya's enhanced prompt
      const mayaPrompt = `Write a ${exercise.suggestedTone} email for ${exercise.audience} about ${exercise.scenario}. Context: ${exercise.audience} needs ${exercise.suggestedTone} communication. Consider their perspective and emotional state.`;
      setAiPopover(prev => ({
        ...prev,
        currentPrompt: mayaPrompt,
        step: 2,
        message: "Building Maya's enhanced prompt with audience context"
      }));
      ...
      const specificPrompt = {
        ...
        audienceContext: exercise.scenario,
        situationDetails: `needs ${exercise.suggestedTone} tone`,
        tone: exercise.suggestedTone,
        aiPrompt: mayaPrompt
      };
      ...
      // Show popover step 3 - AI Processing
      setAiPopover(prev => ({
        ...prev,
        step: 3,
        message: "AI processing both prompts to generate responses"
      }));
      ...
      // Show popover step 4 - Results comparison
      setAiPopover(prev => ({
        ...prev,
        step: 4,
        message: "Comparing results: Generic vs Maya's audience-specific approach"
      }));
      
      // Wait for step 4 to be fully visible (shorter for final step)
      await new Promise(resolve => setTimeout(resolve, 1500));
      ...
      setToneMasteryJourney(prev => ({
        ...prev,
        isGeneratingAI: false,
        practiceGenericResponse: genericResult.email,
        practiceSpecificResponse: specificResult.email,
        adaptedMessage: specificResult.email
      }));
      
    } catch (error) {
      console.error('AI practice generation failed:', error);
      
      // Wait for step 4 to be visible before hiding (shorter timing)
      await new Promise(resolve => setTimeout(resolve, 1500));
      ...
      setToneMasteryJourney(prev => ({
        ...prev,
        isGeneratingAI: false,
        aiError: 'Failed to generate AI practice examples. Using fallback examples.',
        practiceGenericResponse: `Generic response: ${exercise.originalMessage}`,
        practiceSpecificResponse: exercise.adaptedMessage,
  ...
  // Clear messages when transitioning to new stage
  const clearMessagesForNewStage = React.useCallback(() => {
    setVisibleMessages([]);
    setTypedContent({});
    setIsTyping(null);
    messageTimeoutsRef.current.forEach(clearTimeout);
          ...
          <div className="space-y-4">
            {audienceProfiles.map((audience) => (
              <Card 
                ...
                onClick={() => {
                  // Auto-fastforward Lyra chat when selection is made
                  handleFastForward();
              ...
              <Card className="border-yellow-300 bg-yellow-50">
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium text-yellow-800">AI Generation Issue</span>
                  </div>
                  <p className="text-sm text-yellow-700">{toneMasteryJourney.aiError}</p>
                </CardContent>
              </Card>
            </motion.div>
          )}
            ...
            <div className="space-y-6">
              ...
              {!toneMasteryJourney.isGeneratingAI && (toneMasteryJourney.genericResponse || toneMasteryJourney.badToneExample) && (
                ...
                >
                  ...
                  <Card className="border-green-400 bg-gradient-to-r from-green-100 to-emerald-100 shadow-lg mb-6">
                    <CardContent className="p-4">
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <div className="text-center">
                          <h3 className="text-lg font-bold text-green-800">✨ AI Generation Complete!</h3>
                          <p className="text-sm text-green-700">See how different prompts create dramatically different results</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                ...
                <CardContent>
                  <div className="mb-3">
                    ...
                    <p className="text-sm text-gray-600 bg-white p-2 rounded border italic">
                      "Write a professional email about thanking a volunteer for their help with a community event" <span className="text-red-600">(No audience context!)</span>
                    </p>
                  </div>
                  <div>
                    <Badge variant="outline" className="text-xs mb-2">AI Result</Badge>
                    <div className="text-sm bg-white p-4 rounded border leading-relaxed">
                      ...
                      <p className="mb-3">
                        I hope this message finds you well. I wanted to reach out to thank you for your assistance with our recent community event.
                      </p>
                      <p className="mb-3">
                        Your participation was valuable and helped make the event successful. We appreciate when community members like yourself take time to support our initiatives.
                      </p>
                      <p className="mb-3">
                        The event went well and we had good turnout from the community. Your contribution was part of that success.
                      </p>
                      <p className="mb-3">
                        Thank you again for your time and effort. We hope to work with you on future community events.
                      </p>
                      <p className="mb-2">
                        Sincerely,<br/>
                        Maya Rodriguez<br/>
              ...
              <Card className="border-green-200 bg-green-50">
                ...
                <CardContent>
                  ...
                  <div>
                    ...
                    <div className="text-sm bg-white p-4 rounded border leading-relaxed">
                      ...
                      <p className="mb-3">
                        I know you're on the go, so here are a few important updates:
                      </p>
                      <ul className="mb-3 ml-4 space-y-1">
                        <li className="flex items-start">
                          <span className="font-medium text-gray-700 mr-2">•</span>
                          <span><strong>Pick-Up Reminder:</strong> Please remember that our pickup time is at 5:30 PM. If you need to extend this time occasionally, just let us know!</span>
                        </li>
                        <li className="flex items-start">
                          <span className="font-medium text-gray-700 mr-2">•</span>
                          <span><strong>Upcoming Events:</strong> We have a fun Family Arts & Crafts Day coming up on [insert date]. It's a wonderful opportunity for families to connect and create together!</span>
                        </li>
                        <li className="flex items-start">
                          <span className="font-medium text-gray-700 mr-2">•</span>
                          <span><strong>Child's Well-being:</strong> Your child is doing wonderfully in our program! They have been making new friends and really enjoying the activities.</span>
                        </li>
                      </ul>
                      <p className="mb-3">
                      ...
                      <p className="mb-3">
                        Thank you once again for being such a vital part of our community. We appreciate you!
                      </p>
                      <p className="mb-2">
                        Warmly,<br/>
                        Maya Rodriguez<br/>
                      ...
                      <p className="text-xs text-gray-600 italic">
                        P.S. If you have any feedback or suggestions, we'd love to hear from you!
                      </p>
                    </div>
                  </div>
                </CardContent>
                ...
                <Button 
                  onClick={() => {
                    // Clear all messages and timers before transitioning
                    clearMessagesForNewStage();
              ...
              <Card 
                ...
                onClick={() => {
                  ...
                  setToneMasteryJourney(prev => ({ 
                    ...
                    adaptedMessage: exercise.adaptedMessage,
                    isGeneratingAI: true,
                    currentPrompt: `Generate practice example for ${exercise.scenario}`,
                    aiError: ''
                  }));
                  
                  // Generate AI practice examples
                  generatePracticeComparison(exercise);
                  setLyraExpression('celebrating');
                ...
                return (
                  <Card className="bg-gradient-to-r from-green-50 to-emerald-50 border-green-200">
                    ...
                    <CardContent className="space-y-4">
                      {/* Show AI-generated comparison or loading state */}
                      {(toneMasteryJourney.practiceGenericResponse || toneMasteryJourney.practiceSpecificResponse || toneMasteryJourney.isGeneratingAI) && (
                        <div className="space-y-4">
                          ...
                          <div className="bg-gradient-to-r from-blue-100 to-indigo-100 rounded-lg p-3 border border-blue-200">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="text-sm font-bold text-blue-900">Live AI Comparison Results</span>
                            </div>
                            <p className="text-xs text-blue-700">
                              Both responses generated in real-time using different prompt strategies
                            </p>
                          </div>
                          
                          {/* Generic AI Response */}
                          <div>
                            <Badge variant="outline" className="mb-2 bg-red-50 text-red-700">❌ Generic Prompt Result</Badge>
                            <div className="text-xs text-red-600 mb-1 italic">
                              Prompt: "Write a professional email about {(() => {
                                const exercise = toneExercises.find(ex => ex.id === toneMasteryJourney.practiceScenario);
                                return exercise ? exercise.scenario : 'the scenario';
                              })()} " (No audience context)
                            </div>
                            <div className="text-sm bg-red-50 p-4 rounded border border-red-200">
                              {toneMasteryJourney.isGeneratingAI ? (
                                ...
                                formatEmailContent(toneMasteryJourney.practiceGenericResponse || 'Generic response not available')
                              )}
                            </div>
                          </div>
                          
                          {/* Maya's AI-Generated Response */}
                          <div>
                            <Badge variant="outline" className="mb-2 bg-green-50 text-green-700">✅ Maya's Enhanced Prompt Result</Badge>
                            <div className="text-xs text-green-600 mb-1 italic">
                              Prompt: "Write a {(() => {
                                const exercise = toneExercises.find(ex => ex.id === toneMasteryJourney.practiceScenario);
                                return exercise ? `${exercise.suggestedTone} email for ${exercise.audience} about ${exercise.scenario}` : 'enhanced email';
                              })()} with audience context"
                            </div>
                            <div className="text-sm bg-green-50 p-4 rounded border border-green-200">
                              {toneMasteryJourney.isGeneratingAI ? (
                                <div className="flex items-center gap-2 text-gray-500">
                                  <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-green-600"></div>
                                  <span>Generating audience-specific response...</span>
                                </div>
                              ) : (
                                formatEmailContent(toneMasteryJourney.practiceSpecificResponse || toneMasteryJourney.adaptedMessage)
                              )}
                            </div>
                          </div>
                        </div>
                      ...
                      {/* Fallback to hardcoded only if AI not available AND not generating */}
                      {!toneMasteryJourney.practiceGenericResponse && !toneMasteryJourney.practiceSpecificResponse && !toneMasteryJourney.isGeneratingAI && (
                        <div>
                          <Badge variant="outline" className="mb-2">Maya's Adapted Message</Badge>
                          <div className="text-sm bg-white p-4 rounded border border-green-200">
                            {formatEmailContent(selectedExercise.adaptedMessage)}
                          </div>
                        </div>
                      )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-center">
                      <SaveToToolkit
                        ...
                        emailContent={toneMasteryJourney.practiceSpecificResponse || toneMasteryJourney.adaptedMessage}
                        promptBuilder={{
  ...
  useEffect(() => {
    if (currentStage && currentStage.narrativeMessages) {
      // Clear previous messages when transitioning to new stage
      clearMessagesForNewStage();
      // Process new stage messages after a brief delay
      setTimeout(() => {
        processMessages(currentStage.narrativeMessages);
      }, 100);
    }
  }, [currentStageIndex, processMessages, currentStage, clearMessagesForNewStage]);
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-pink-50 via-white to-purple-50 flex flex-col">
      ...
      <AIGenerationPopover 
        isVisible={aiPopover.isVisible}
        currentPrompt={aiPopover.currentPrompt}
        step={aiPopover.step}
        totalSteps={aiPopover.totalSteps}
        message={aiPopover.message}
      />
