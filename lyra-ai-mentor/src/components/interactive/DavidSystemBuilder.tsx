...
interface DavidSystemBuilderProps {
  onComplete?: () => void;
}

export const DavidSystemBuilder: React.FC<DavidSystemBuilderProps> = ({ onComplete }) => {
  const { toast } = useToast();
  const [currentPhase, setCurrentPhase] = useState<'context' | 'design' | 'build' | 'automate' | 'success'>('context');
  const [userInput, setUserInput] = useState('');
  const [enhancedOutput, setEnhancedOutput] = useState('');

  const enhanceWithAI = async () => {
    setCurrentPhase('automate');
    
    const enhancement = `${userInput}

AI ENHANCEMENT APPLIED:
✨ AI dashboard tools and automated reporting systems
...
This approach demonstrates scalable system that turns data into insights automatically through systematic application of proven methodologies.`;

    setEnhancedOutput(enhancement);
    
    toast({
      title: "✨ Enhancement Complete!",
      description: "Scalable system that turns data into insights automatically",
    });
  };
  ...
  const handleComplete = async () => {
    ...
    if (session?.user) {
      await supabase.from('interactive_element_progress').insert({
        ...
        element_id: 'david-system-builder',
        lesson_id: 18,
        completed: true,
        ...
        return (
          <div className="space-y-6">
            <div className="text-center space-y-4">
              ...
              <h2 className="text-2xl font-bold">Data Storytelling System Builder</h2>
              <div className="max-w-2xl mx-auto text-gray-600 space-y-3">
                <p><strong>Challenge:</strong> Creating sustainable system for ongoing data communication</p>
                <p><strong>Discovery:</strong> AI dashboard tools and automated reporting systems</p>
                <p><strong>Practice:</strong> Building comprehensive data storytelling infrastructure</p>
              </div>
            </div>
      ...
      case 'automate':
        return (
          <div className="space-y-6">
            ...
            <Card className="border-2 border-blue-200 bg-blue-100">
          ...
          <div className="text-center space-y-6 py-8">
            ...
            <div className="space-y-4">
              ...
              <p className="text-gray-600 max-w-md mx-auto">
                Scalable system that turns data into insights automatically
              </p>
            </div>
          </div>
        );
