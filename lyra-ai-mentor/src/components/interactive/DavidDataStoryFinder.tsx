          ...
          <div className="space-y-6">
            ...
            {storyTemplate && (
              <DavidLiveCharts 
                ...
                animationSpeed={1200}
                showComparison={storyTemplate === 'The Transformation Journey'}
              />
            )}
        ...
        {currentPhase === 'build' && (
          <div className="space-y-6">
            ...
            <div className="flex gap-3">
              <Button onClick={enhanceStory} disabled={!dataStory} className="flex-1 bg-green-600 hover:bg-green-700">
                <Sparkles className="w-4 h-4 mr-2" /> Enhance Story
              </Button>
              <ExportButton
                data={(): ExportData => ({
                  title: `Data Story - ${storyTemplate} - ${new Date().toLocaleDateString()}`,
                  content: dataStory,
                  metadata: {
                    createdAt: new Date().toISOString(),
                    author: 'David Chen',
                    tags: ['data-story', 'analytics', storyTemplate],
                    template: storyTemplate
                  },
                  sections: [
                    {
                      title: 'Story Template',
                      content: templates.find(t => t.name === storyTemplate)?.pattern || '',
                      type: 'text'
                    },
                    {
                      title: 'Data Story',
                ...
                className="flex-1"
                characterName="David"
                suggestUseIn={['Alex Strategy Planning', 'Maya Grant Proposals', 'Presentations']}
              />
            </div>
            ...
            <UseInSuggestions
              content={{
                story: dataStory,
                template: storyTemplate,
                templateDetails: templates.find(t => t.name === storyTemplate)
              }}
              contentType="data"
              fromCharacter="David"
