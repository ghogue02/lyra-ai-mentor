...
-- Get category IDs and insert toolkit items
DO $$
DECLARE
...
BEGIN
    ...
    -- Check if items already exist and insert only if they don't
    IF NOT EXISTS (SELECT 1 FROM toolkit_items WHERE name = 'PACE Framework Master Template') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM toolkit_items WHERE name = 'Professional Email Crisis Template') THEN
        INSERT INTO toolkit_items (
            name, 
            category_id, 
            description, 
    ...
    IF NOT EXISTS (SELECT 1 FROM toolkit_items WHERE name = 'Multi-Audience Tone Adaptation Guide') THEN
        INSERT INTO toolkit_items (
            name, 
            category_id, 
            description, 
    ...
    IF NOT EXISTS (SELECT 1 FROM toolkit_items WHERE name = 'Crisis Communication Multi-Template Pack') THEN
        INSERT INTO toolkit_items (
            name, 
            category_id, 
            description, 
