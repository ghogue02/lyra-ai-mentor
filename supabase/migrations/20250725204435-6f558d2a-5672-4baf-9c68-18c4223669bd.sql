...
-- Drop the existing check constraint on criteria_type
ALTER TABLE public.toolkit_achievements DROP CONSTRAINT IF EXISTS toolkit_achievements_criteria_type_check;

-- Add updated check constraint with new criteria types
ALTER TABLE public.toolkit_achievements ADD CONSTRAINT toolkit_achievements_criteria_type_check 
CHECK (criteria_type IN ('category_count', 'unlock_count', 'journey_completion', 'journey_score', 'journey_avg_score', 'journey_min_score', 'cross_journey', 'journey_speed'));
