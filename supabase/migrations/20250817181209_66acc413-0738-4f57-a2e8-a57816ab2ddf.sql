-- Add Carmen character type and update content types to include existing ones
...
-- Update content type constraint to include all existing and new HR-specific types
ALTER TABLE generated_content 
DROP CONSTRAINT IF EXISTS generated_content_content_type_check;

ALTER TABLE generated_content 
ADD CONSTRAINT generated_content_content_type_check 
CHECK (content_type = ANY (ARRAY['email'::text, 'lesson'::text, 'document'::text, 'article'::text, 'ecosystem-blueprint'::text, 'story'::text, 'analysis'::text, 'automation'::text, 'transformation'::text, 'hr_template'::text, 'evaluation_framework'::text, 'recruitment_tool'::text, 'team_analysis'::text, 'retention_model'::text]));
