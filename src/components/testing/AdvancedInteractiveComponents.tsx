...
import { Users, Target, MapPin, Heart, BarChart, Gamepad2, Timer, CheckCircle, ArrowRight, Shuffle, Star, DollarSign, Clock, TrendingUp, Award, RefreshCw, Trophy, Loader2 } from 'lucide-react';
...
export const DonorSegmentationSimulator = () => {
  ...
  return <div className="space-y-4">
      ...
      <div className="space-y-3">
        {donors.map(donor => <Card key={donor.id} className="border border-gray-200">
            <CardContent className="p-4">
              <div className="flex justify-between items-start mb-3">
                ...
                {assignments[donor.id] && <Badge className={`text-xs ${showResults ? assignments[donor.id] === donor.correctSegment ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>
                    {assignments[donor.id]}
                  </Badge>}
              </div>
              
              {!showResults && <div className="flex flex-wrap gap-1">
                  {segments.map(segment => <Button key={segment} onClick={() => handleAssignment(donor.id, segment)} variant={assignments[donor.id] === segment ? "default" : "outline"} size="sm" className="text-xs h-6">
                      {segment}
                    </Button>)}
                </div>}
            </CardContent>
      ...
      <div className="flex gap-2">
        <Button onClick={checkResults} disabled={!allAssigned || showResults} size="sm">
      ...
      {showResults && <Card className="border border-blue-200">
...
export const VolunteerCoordinationGame = () => {
  ...
  return <div className="space-y-4">
      ...
      {!isActive && !gameOver && <Button onClick={startGame} className="w-full">
      ...
      {isActive && <div className="space-y-3">
          <div>
            ...
            {tasks.map(task => <Card key={task.id} className="border border-gray-200 mb-2">
                <CardContent className="p-3">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <div className="flex gap-1 mt-1">
                        ...
                        <Badge className={`text-xs ${task.urgency === "High" ? "bg-red-100 text-red-700" : task.urgency === "Medium" ? "bg-orange-100 text-orange-700" : "bg-green-100 text-green-700"}`}>
                          {task.urgency}
                        </Badge>
                      </div>
                    </div>
                    {assignments[task.id] && <Badge className="bg-blue-100 text-blue-700">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {volunteers.map(volunteer => <Button key={volunteer.id} onClick={() => {
            // Find unassigned high priority task first
            const unassignedTask = tasks.find(task => !assignments[task.id] && volunteer.skills.includes(task.skill));
            if (unassignedTask) {
              assignVolunteer(unassignedTask.id, volunteer.id);
            }
          }} variant="outline" size="sm" className="text-xs" disabled={Object.values(assignments).includes(volunteer.id)}>
                  {volunteer.name}
      ...
      {gameOver && <Card className="border border-green-200">
          <CardContent className="p-4">
            <div className="text-center">
              ...
              <p className="text-xs text-gray-500">
                {score >= 8 ? "Excellent coordination!" : score >= 5 ? "Good job managing volunteers!" : "Keep practicing coordination skills!"}
              </p>
...
export const FoodRescueRouteOptimizer = () => {
  ...
  return <div className="space-y-4">
      ...
      <div className="space-y-3">
        <div>
          ...
          <div className="space-y-2">
            {locations.map(location => <Card key={location.id} className="border border-gray-200">
                <CardContent className="p-3">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <div className="flex gap-1 mt-1">
                        ...
                        {location.urgency > 0 && <Badge className={`text-xs ${location.urgency === 3 ? "bg-red-100 text-red-700" : location.urgency === 2 ? "bg-orange-100 text-orange-700" : "bg-yellow-100 text-yellow-700"}`}>
                    ...
                    <div className="flex gap-1">
                      {selectedRoute.includes(location.id) ? <Button onClick={() => removeFromRoute(location.id)} variant="default" size="sm" className="text-xs h-6">
                          #{selectedRoute.indexOf(location.id) + 1}
                        </Button> : <Button onClick={() => addToRoute(location.id)} variant="outline" size="sm" className="text-xs h-6">
                          Add to Route
                        </Button>}
                    </div>
                  </div>
        ...
        {selectedRoute.length > 0 && <Card className="border border-blue-200">
            <CardContent className="p-3">
              <h4 className="text-sm font-medium mb-2">Your Route:</h4>
              <div className="flex items-center gap-1 flex-wrap">
                {selectedRoute.map((locationId, index) => {
              const location = locations.find(l => l.id === locationId);
              return <React.Fragment key={locationId}>
                      <Badge className="bg-blue-100 text-blue-700 text-xs">
                        {location?.name}
                      </Badge>
                      {index < selectedRoute.length - 1 && <ArrowRight className="w-3 h-3 text-gray-400" />}
                    </React.Fragment>;
            })}
              </div>
            </CardContent>
      ...
      <div className="flex gap-2">
        <Button onClick={calculateEfficiency} disabled={selectedRoute.length < 3 || optimized} size="sm">
      ...
      {optimized && <Card className="border border-green-200">
          <CardContent className="p-3">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs text-green-700">
              {efficiency >= 90 ? "Excellent route optimization!" : efficiency >= 70 ? "Good route planning!" : "Consider prioritizing urgent pickups and minimizing distance."}
            </p>
          </CardContent>
        </Card>}
    </div>;
...
export const MentorMatchingSimulator = () => {
  ...
  const mentors = [{
    id: "sarah",
    name: "Sarah Wilson",
    skills: ["Technology", "Career Development"],
    schedule: "Weekends",
    personality: "Patient"
  }, {
    id: "james",
    name: "James Chen",
    skills: ["Arts", "Creative Writing"],
    schedule: "Evenings",
  ...
  return <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          ...
          {mentees.map(mentee => <Card key={mentee.id} className="border border-gray-200 mb-2">
              <CardContent className="p-3">
                <div className="flex justify-between items-start">
                  ...
                  {matches[mentee.id] && <Badge className={`text-xs ${showResults ? matches[mentee.id] === mentee.optimalMentor ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>
                      {mentors.find(m => m.id === matches[mentee.id])?.name.split(' ')[0]}
                    </Badge>}
                </div>
                {!showResults && <div className="mt-2">
                    {matches[mentee.id] ? <Button onClick={() => removeMatch(mentee.id)} variant="outline" size="sm" className="text-xs h-6">
                        Change Match
                      </Button> : <div className="flex flex-wrap gap-1">
                        {mentors.filter(mentor => !Object.values(matches).includes(mentor.id)).map(mentor => <Button key={mentor.id} onClick={() => createMatch(mentee.id, mentor.id)} variant="outline" size="sm" className="text-xs h-6">
                            {mentor.name.split(' ')[0]}
                          </Button>)}
                      </div>}
                  </div>}
        ...
        <div>
          ...
          {mentors.map(mentor => <Card key={mentor.id} className={`border mb-2 ${Object.values(matches).includes(mentor.id) ? 'border-blue-300 bg-blue-50' : 'border-gray-200'}`}>
              <CardContent className="p-3">
                ...
                {Object.values(matches).includes(mentor.id) && <Badge className="bg-blue-100 text-blue-700 text-xs mt-1">
                    Matched
                  </Badge>}
              </CardContent>
            </Card>)}
      ...
      <div className="flex gap-2">
        <Button onClick={checkMatches} disabled={!allMatched || showResults} size="sm">
      ...
      {showResults && <Card className="border border-green-200">
          <CardContent className="p-3">
            ...
            <p className="text-xs text-green-700">
              {score >= 3 ? "Excellent matching! Great mentor-mentee compatibility." : score >= 2 ? "Good matches! Consider interests and personalities." : "Keep practicing! Focus on matching skills with needs."}
            </p>
          </CardContent>
        </Card>}
    </div>;
...
export const AIBeforeAfterSlider = () => {
  ...
  return <div className="space-y-4">
      ...
      <div className="flex gap-1">
        {scenarios.map((scenario, index) => <Button key={index} onClick={() => {
        setSelectedScenario(index);
        setSliderValue(0);
      }} variant={selectedScenario === index ? "default" : "outline"} size="sm" className="text-xs">
            {scenario.title}
          </Button>)}
      </div>
      ...
      <Card className="border border-gray-200">
        <CardContent className="p-4">
          ...
          <div className="mb-4">
            <input type="range" min="0" max="100" value={sliderValue} onChange={e => setSliderValue(parseInt(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
      ...
      <Card className="border border-blue-200">
        <CardContent className="p-3">
          <p className="text-sm text-blue-700">
            <strong>Impact Summary:</strong> {ratio === 0 ? "Traditional manual processes in place." : ratio < 0.5 ? "Early AI adoption showing initial improvements." : ratio < 0.8 ? "Significant AI integration delivering measurable results." : "Full AI implementation maximizing organizational efficiency."}
          </p>
        </CardContent>
      </Card>
    </div>;
...
export const NonprofitAIBingo = () => {
  ...
  const bingoSquares = ["Email Auto", "Donor Predict", "Volunteer Match", "Route Optimize", "Data Analysis", "Chatbot", "Image Recog", "Sentiment", "Fraud Detect", "Content AI", "Translation", "Schedule AI", "Budget Forecast", "Impact Measure", "Social Media AI", "Voice Recog", "Doc Process", "Grant Writing", "Inventory AI", "Recruit AI", "Event Planning", "Survey Analysis", "Compliance", "Performance", "Comm AI"];
  const boards = [bingoSquares.slice(0, 25), [...bingoSquares.slice(10, 25), ...bingoSquares.slice(0, 10)], bingoSquares.slice().sort(() => Math.random() - 0.5).slice(0, 25)];
  ...
  return <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
        {currentBoardSquares.map((square, index) => <Button key={`${currentBoard}-${index}`} onClick={() => toggleSquare(square)} variant={markedSquares.has(square) ? "default" : "outline"} className="h-20 sm:h-22 md:h-24 text-xs sm:text-sm p-3 flex items-center justify-center leading-relaxed whitespace-normal" size="sm">
      ...
      {hasBingo && <Card className="border border-yellow-200">
          <CardContent className="p-4 text-center">
            ...
            {loadingSummary ? <div className="flex items-center justify-center gap-2 mb-3">
                <Loader2 className="w-4 h-4 animate-spin text-blue-600" />
                <span className="text-sm text-blue-600">Generating AI implementation guide...</span>
              </div> : aiSummary && <Card className="border border-blue-200 mb-3">
                <CardContent className="p-3">
