...
import { CheckCircle, Circle, Lock, Rocket, Sparkles, Home } from 'lucide-react';
import { cn } from '@/lib/utils';
import { supabase } from '@/integrations/supabase/client';
import { useAuth } from '@/contexts/AuthContext';
import { BrandedButton } from '@/components/ui/BrandedButton';

interface Chapter6SidebarProps {
...
const lessons = [
  { 
    ...
    subtitle: "Alex faces resistance",
    icon: "🌉",
    color: "gray"
  },
  { 
    ...
    subtitle: "Uniting the organization",
    icon: "🎯",
    color: "purple"
  },
  { 
  ...
  { 
    ...
    subtitle: "AI-powered leadership",
    icon: "🚀",
    color: "indigo"
  }
];

export const Chapter6Sidebar: React.FC<Chapter6SidebarProps> = ({ 
  currentLessonId,
  onLessonChange 
}) => {
  ...
  const handleLessonClick = (lessonId: number) => {
    if (onLessonChange) {
      ...
      navigate(`/chapter/6/lesson/${lessonId}`);
    }
  };
  ...
  return (
    <div className="w-80 h-screen bg-gradient-to-b from-purple-50 to-indigo-50 border-r border-purple-200 flex flex-col">
      {/* Chapter Header */}
      <div className="p-6 border-b border-purple-200 bg-white/80 backdrop-blur-sm">
        <div className="flex items-center gap-3 mb-2">
          <Rocket className="w-6 h-6 text-purple-500" />
          <h2 className="text-lg font-bold text-gray-900">Chapter 6</h2>
        </div>
        <h3 className="text-xl font-bold bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent">
          Organizational Transformation
        </h3>
        <p className="text-sm text-gray-600 mt-2">
          Join Alex as they lead AI-powered change
        </p>
      </div>
            ...
            <button
              ...
              className={cn(
                ...
                isActive ? [
                  ...
                  lesson.color === 'violet' && "border-violet-400 bg-violet-50",
      ...
      <div className="p-4 border-t border-purple-200">
        <BrandedButton 
          ...
          variant="outline" 
          className="w-full flex items-center gap-2 hover:bg-purple-50 border-purple-200"
          icon="mission"
          animated={true}
        >
          Return to Dashboard
      ...
      <div className="p-4 bg-gradient-to-r from-purple-100 to-indigo-100 border-t border-purple-200">
        <div className="flex items-start gap-2">
          ...
          <p className="text-sm italic text-gray-700">
            "True transformation isn't about the technology. It's about empowering every person to reach their full potential with AI as their partner."
          </p>
        </div>
        <p className="text-xs text-gray-600 mt-1 text-right">— Alex Rivera</p>
      </div>
    </div>
  );
};